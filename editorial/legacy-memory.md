...
- Quake VR (A-tier, March 29)
- Space Channel 5 VR (C-tier, March 29)
- Top 10 Retro Games ranking (March 29)
- Into the Radius VR (A-tier, March 31)
...
**CRITICAL: Maya uses Hermes `image_generate` tool for hero image generation.**

The built-in `image_generate` tool uses FAL.ai's FLUX 2 Pro model with automatic upscaling.

**Prerequisites:**
- `FAL_KEY` must be configured in Maya's `.env` at `/home/antforce/.hermes/profiles/maya/.env`
...
Use the image_generate tool with appropriate prompt. Reference the CompoundVR brand guide at /design/compoundvr-brand-direction-v1.md for visual direction (deep navy #0b1020, cyan accent #6ee7ff, cinematic moody atmosphere).
...
**Visual direction prompts should include:**
- Dark, moody atmosphere
- CompoundVR brand colors (deep navy, cyan accent)
- Cinematic 16:9 wide composition
- Game-appropriate theming

**Critical:**
- If `FAL_KEY` is not set, image generation will fail with "FAL_KEY environment variable not set"
- Verify output is valid JPEG with recognizable content
- Convert PNG to JPEG if needed (FLUX outputs PNG by default)
...
**Into the Radius VR** — Published 2026-03-31
- B-tier verdict: Unforgiving hardcore survival shooter, atmospheric and immersive
- Native VR: Quest 2/3, PSVR2, PCVR
- Extraction shooter with permadeath, weapon maintenance, anomaly hazards
- RouteType: Official Standalone VR Version
- Live at: https://compoundvr.com/games/into-the-radius
