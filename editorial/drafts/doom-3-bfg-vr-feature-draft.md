...
For players who've experienced modern VR games with complex gesture systems and radial menus, voice commands might feel dated. But in 2017, this was forward-thinking experimentation. The developers were asking: what else can we do with the hardware? The microphone was there. Why not use it?
...
**Game Quality:** B+ — Doom 3 is a competent corridor shooter with effective but dated horror design. The campaign is substantial but linear, the story is forgettable, and the jump-scare reliance hasn't aged well. The BFG Edition package adds value with included expansions and classic Doom games.

**VR Implementation Quality:** A- — The motion controls, room-scale support, comfort options, and thoughtful features like holster slots and voice commands represent some of the most comprehensive VR conversion work ever done on a retail game. The few incomplete features and basic IK are the only real knocks.

**Historical Significance:** S — One of the first complete game-to-VR conversions, predating better-known mods, and establishing standards for comfort options, motion control integration, and feature completeness that many later mods didn't match.

**Maintenance Status:** B+ — Original development stopped, but community fork continues bug fixes and compatibility updates. Stable and playable on current hardware.
...
*Platform: PC VR (SteamVR, Oculus SDK) | Input: Motion Controllers, Gamepad, Keyboard/Mouse | Source Game: Doom 3 BFG Edition (Steam) | Developer: KozGit with contributions from Carl Kenner and others | Community Fork: NPi2Loup | Cost: Free (requires base game)*